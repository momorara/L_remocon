...
air_set03 -> remocon_mode03_02.py-> remocon_mode03_03.py
...
def print_message():
    log_print ('|********************************|')
    log_print ('| remocon_mode03_03.py    4     |')
    log_print ('|********************************|')
...
def makeModeFile(mode):
    with open(path + 'modeFile.txt', 'a') as f:
        mode += '\n'
        f.write(mode)
    #makeModeFile('mode選択')
...
def iR_send(send_comand):
    try:
        ...
        send_comand = 'python3 ' + path + 'irrp.py -p -g' + iR_LED + ' -f ' + send_comand
        # 設定変更のスタート時間を記憶
